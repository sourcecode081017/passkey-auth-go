...
function PasskeyRegisterAuthenticate({ username, onAuthSuccess }: RegisterAuthProps) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/passkey-auth/register-initiate/${encodeURIComponent(
          username
        )}`,
        {
          method: "POST",
      ...
      const verifyResponse = await fetch(
        `http://localhost:8080/passkey-auth/register-complete/${encodeURIComponent(
          username
        )}`,
        {
          method: "POST",
  ...
  const handleAuthenticate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/passkey-auth/auth-initiate/${encodeURIComponent(
          username
        )}`,
        {
          method: "POST",
      ...
      // Step 4: Send the credential to your server for verification
      const verifyResponse = await fetch(
        `http://localhost:8080/passkey-auth/auth-complete/${encodeURIComponent(
          username
        )}`,
        {
          method: "POST",
